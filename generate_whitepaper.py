...
filename = os.path.join(docs_dir, "ARCHITECTURE_WHITEPAPER.md")

content_part1 = """# ARCHITECTURE & ENGINEERING WHITEPAPER
...
$$ EMA_{t} = \alpha \cdot Value_t + (1 - \alpha) \cdot EMA_{t-1} $$
Where $\alpha$ (the smoothing factor) is dynamically calculated based on the distance between the old and new coordinates. If the distance is massive (the person moved rapidly), $\alpha \rightarrow 1$ (trust new value). If the distance is tiny (sensor noise), $\alpha \rightarrow 0.1$ (trust old value).
...
print(f"Whitepaper successfully written to {filename}")